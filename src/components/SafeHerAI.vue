...
const props = defineProps({ location: Object, contacts: { type: Array, default: () => [] } });
const emit = defineEmits(["request-location", "activate-sos", "contact-trusted", "start-journey"]);
...
<template>
  <div class="safeher-ai" :class="{ open, emergency: emergencyMode }">
    <button v-if="!open" class="ai-launcher" aria-label="Open Safe_Her AI" @click="open = true"><i class="bi bi-shield-fill"></i><span>Safe_Her AI</span></button>
