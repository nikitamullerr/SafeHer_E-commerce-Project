...
<template>
  <div class="safeher-ai" :class="{ open, emergency: emergencyMode }">
    <button v-if="!hasPremiumAccess" class="ai-launcher" :aria-label="t(&quot;Unlock SafeHer AI&quot;)" @click="emit('upgrade')"><i class="bi bi-chat-dots-fill"></i><span>{{ t("Unlock AI") }}</span></button>
    <button v-else-if="!open" class="ai-launcher" :aria-label="t(&quot;Open Safe_Her AI&quot;)" @click="open = true"><i class="bi bi-chat-dots-fill"></i><span>Safe_Her AI</span></button>
    <section v-else class="ai-window" :aria-label="t(&quot;Safe_Her AI chat&quot;)">
      ...
      <p class="privacy-note">{{ t("Location is requested only when you choose it. This chat does not place emergency calls.") }}</p>
    </section>
  </div>
</template>
